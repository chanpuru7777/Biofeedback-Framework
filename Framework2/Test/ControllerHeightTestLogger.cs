...
public class ControllerHeightTestLogger : MonoBehaviour
{
    public SharedData sharedData; // データ共有スクリプト
    public string csvFilePath = "TestDataLog_ControllerHeight.csv"; // 保存先のCSVファイル名
    ...
    void Update()
    {
        ...
        if (sharedData.inputDataA.Count > 0 && sharedData.processedDataB.Count > 0)
        {
            ...
            float controllerHeight = sharedData.inputDataA[sharedData.inputDataA.Count - 1][3]; // コントローラ高さ（A_Level）
            float awarenessLevel = sharedData.processedDataB[sharedData.processedDataB.Count - 1][0]; // 処理後データ（B_Level）

            // データをCSVファイルに書き込む
            writer.WriteLine($"{frame},{timestamp},{controllerHeight},{awarenessLevel}");
        }
    }
